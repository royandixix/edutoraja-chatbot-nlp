...
if (isset($_POST['reset'])) {
    ...
    if ($res->num_rows == 0) {
        $error = "Username tidak ditemukan di sistem kami.";
    } else {
...
<head>
    ...
    <title>Lupa Password | EduToraja</title>
...
<body class="bg-surface font-body text-on-surface antialiased min-h-screen flex items-center justify-center relative p-4">
    ...
    <div class="bg-surface-container-lowest rounded-2xl shadow-lg border border-outline-variant/30 w-full max-w-md p-8 relative z-10">
        ...
        <div class="text-center mb-8">
            ...
            <h2 class="font-headline text-2xl font-bold text-on-surface tracking-tight">Lupa Password</h2>
            <p class="text-sm text-on-surface-variant mt-1">Atur ulang password untuk akun Anda</p>
        </div>
        ...
        <form method="POST" action="">
            <div class="mb-5">
                <label class="block text-sm font-bold text-on-surface-variant mb-2">Username Anda</label>
                <div class="relative">
            ...
            <div class="mb-8">
                <label class="block text-sm font-bold text-on-surface-variant mb-2">Password Baru</label>
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant opacity-60">lock_reset</span>
                    <input type="password" name="new_password" class="w-full pl-10 pr-4 py-3 rounded-xl border border-outline-variant/50 focus:border-primary focus:ring-primary text-sm bg-surface-container-low transition-colors" required placeholder="Masukkan password baru">
                </div>
            </div>
            
            <button type="submit" name="reset" class="w-full py-3.5 bg-primary text-on-primary rounded-xl font-bold hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
        ...
        <div class="mt-8 pt-6 border-t border-outline-variant/20 flex flex-col gap-4 text-center">
            <a href="index.php" class="text-sm font-medium text-on-surface-variant hover:text-on-surface inline-flex items-center justify-center gap-2 mt-2 transition-colors">
                <span class="material-symbols-outlined text-sm">west</span> Kembali ke Login
            </a>
        </div>
