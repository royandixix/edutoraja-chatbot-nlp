...
$result = $conn->query("SELECT * FROM budaya ORDER BY id DESC");
$count_budaya = $result->num_rows;
...
<head>
    ...
    <title>Kelola Budaya Adat | EduToraja</title>
...
<aside class="h-screen w-64 fixed left-0 top-0 bg-stone-100 flex flex-col py-6 z-50">
    ...
    <nav class="flex-1 space-y-1">
        ...
        <a class="bg-stone-50 text-orange-900 font-bold rounded-l-full ml-4 pl-4 py-3 flex items-center gap-3 transition-all duration-200" href="budaya.php">
    ...
    <div class="px-6 mt-auto">
        <button onclick="openModal('addModal')" class="w-full py-3 px-4 bg-primary text-on-primary rounded-full flex items-center justify-center gap-2 text-sm font-medium hover:opacity-90 transition-all duration-200">
            <span class="material-symbols-outlined">add</span> Tambah Budaya
        </button>
...
<main class="ml-64 min-h-screen p-10 relative">
    ...
    <header class="flex justify-between items-end mb-12 relative z-10">
        <div>
            <h2 class="font-headline text-4xl text-on-surface font-bold">Kelola Budaya Adat</h2>
            <p class="font-body text-on-surface-variant mt-2">Tambahkan konten tradisi dan kebudayaan adat Toraja.</p>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-xl overflow-hidden shadow-sm border border-outline-variant/30">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low border-b border-outline-variant/30">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Nama Budaya</th>
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    ...
                    <tr class="hover:bg-surface-container-low/30 transition-colors">
                        ...
                        <td class="px-6 py-5 text-right space-x-2">
                            <button onclick="openEditModal(<?= $row['id'] ?>, `<?= htmlspecialchars($row['nama'], ENT_QUOTES) ?>`, `<?= htmlspecialchars($row['deskripsi'], ENT_QUOTES) ?>`, `<?= htmlspecialchars($row['slug'], ENT_QUOTES) ?>`)" class="text-xs font-bold text-primary px-3 py-1.5 border border-primary/20 rounded-full hover:bg-primary hover:text-on-primary transition-all">Edit</button>
                            <a href="proses_budaya.php?action=delete&id=<?= $row['id'] ?>" onclick="return confirm('Yakin ingin menghapus budaya ini dari daftar?');" class="text-xs font-bold text-error px-3 py-1.5 border border-error/20 rounded-full hover:bg-error hover:text-on-error transition-all">Hapus</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            ...
            <?php if($result->num_rows == 0): ?>
                <div class="text-center py-10 text-on-surface-variant">
                    <span class="material-symbols-outlined text-4xl opacity-50">festival</span>
                    <p class="mt-2 text-sm">Belum ada budaya adat yang ditambahkan.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
...
<div class="modal" id="addModal">
    <h3 class="font-headline text-2xl font-bold mb-4 text-on-surface">Tambah Budaya Adat</h3>
    <form action="proses_budaya.php?action=add" method="POST" enctype="multipart/form-data">
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Nama Budaya/Tradisi</label>
            <input type="text" name="nama" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required placeholder="Contoh: Rambu Solo">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Slug (Kata Kunci URL, tanpa spasi)</label>
            <input type="text" name="slug" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required placeholder="Contoh: rambu-solo">
        </div>
        <div class="mb-4">
...
<div class="modal" id="editModal">
    <h3 class="font-headline text-2xl font-bold mb-4 text-on-surface">Edit Budaya Adat</h3>
    <form action="proses_budaya.php?action=edit" method="POST" enctype="multipart/form-data">
...
<script>
    ...
    function openEditModal(id, nama, deskripsi, slug) {
