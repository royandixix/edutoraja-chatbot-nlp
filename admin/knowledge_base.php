...
$count_kb = $result->num_rows;
$res_chat = $conn->query("SELECT COUNT(*) as total FROM chat_logs");
$count_chat = $res_chat->fetch_assoc()['total'];
...
<head>
    ...
    <title>Knowledge Base | EduToraja</title>
...
<aside class="h-screen w-64 fixed left-0 top-0 bg-stone-100 flex flex-col py-6 z-50">
    ...
    <nav class="flex-1 space-y-1">
        ...
        <a class="bg-stone-50 text-orange-900 font-bold rounded-l-full ml-4 pl-4 py-3 flex items-center gap-3 transition-all duration-200" href="knowledge_base.php">
...
<main class="ml-64 min-h-screen p-10 relative">
    ...
    <header class="flex justify-between items-end mb-12 relative z-10">
        <div>
            <h2 class="font-headline text-4xl text-on-surface font-bold">Basis Pengetahuan (TF-IDF)</h2>
            <p class="font-body text-on-surface-variant mt-2">Selamat datang, Admin. Kelola data kecerdasan buatan EduToraja di sini.</p>
        </div>
    ...
    <!-- Stats -->
    <section class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 relative z-10">
        <div class="bg-surface-container-lowest p-6 rounded-xl shadow-sm">
            <div class="flex justify-between items-start mb-4">
                <div class="p-3 bg-primary-fixed rounded-full text-on-primary-fixed-variant">
                    <span class="material-symbols-outlined">dataset</span>
                </div>
            </div>
            <h3 class="text-on-surface-variant text-sm font-medium">Total Knowledge Base</h3>
            <p class="text-3xl font-headline font-bold text-on-surface"><?= $count_kb ?></p>
        </div>
        <div class="bg-surface-container-lowest p-6 rounded-xl shadow-sm">
            <div class="flex justify-between items-start mb-4">
                <div class="p-3 bg-secondary-fixed rounded-full text-on-secondary-fixed-variant">
                    <span class="material-symbols-outlined">forum</span>
                </div>
            </div>
            <h3 class="text-on-surface-variant text-sm font-medium">Total Interaksi Chat</h3>
            <p class="text-3xl font-headline font-bold text-on-surface"><?= $count_chat ?></p>
        </div>
        <div class="bg-primary text-on-primary p-6 rounded-xl relative overflow-hidden">
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-4">
                    <div class="p-3 bg-on-primary/10 rounded-full">
                        <span class="material-symbols-outlined">auto_awesome</span>
                    </div>
                </div>
                <h3 class="text-on-primary/80 text-sm font-medium">Status NLP Engine</h3>
                <p class="text-2xl font-headline font-bold mt-1">Aktif & Optimal</p>
            </div>
        ...
        <div class="bg-surface-container-lowest rounded-xl overflow-hidden shadow-sm border border-outline-variant/30">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low border-b border-outline-variant/30">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">Keywords / Pertanyaan</th>
                        <th class="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider w-1/2">Respons Edukatif</th>
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    ...
                    <tr class="hover:bg-surface-container-low/30 transition-colors">
                        <td class="px-6 py-5 text-sm font-medium text-on-surface-variant">#<?= $row['id'] ?></td>
                        <td class="px-6 py-5">
                            <p class="font-bold text-sm text-on-surface"><?= htmlspecialchars($row['question']) ?></p>
                        </td>
                        <td class="px-6 py-5">
                            <p class="text-sm text-on-surface-variant line-clamp-2"><?= htmlspecialchars($row['answer']) ?></p>
                        </td>
                        <td class="px-6 py-5 text-right space-x-2">
                            <button onclick="openEditModal(<?= $row['id'] ?>, `<?= htmlspecialchars($row['question'], ENT_QUOTES) ?>`, `<?= htmlspecialchars($row['answer'], ENT_QUOTES) ?>`, `<?= htmlspecialchars($row['maps_url'] ?? '', ENT_QUOTES) ?>`)" class="text-xs font-bold text-primary px-3 py-1.5 border border-primary/20 rounded-full hover:bg-primary hover:text-on-primary transition-all">Edit</button>
                            <a href="proses_kb.php?action=delete&id=<?= $row['id'] ?>" onclick="return confirm('Hapus data ini?');" class="text-xs font-bold text-error px-3 py-1.5 border border-error/20 rounded-full hover:bg-error hover:text-on-error transition-all">Hapus</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            ...
            <?php if($result->num_rows == 0): ?>
                <div class="text-center py-10 text-on-surface-variant">
                    ...
                    <p class="mt-2 text-sm">Belum ada data knowledge base.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
...
<div class="modal" id="addModal">
    <h3 class="font-headline text-2xl font-bold mb-4 text-on-surface">Tambah Data</h3>
    <form action="proses_kb.php?action=add" method="POST">
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Pertanyaan / Kata Kunci</label>
            <textarea name="question" rows="3" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required></textarea>
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Jawaban Edukatif</label>
            <textarea name="answer" rows="5" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required></textarea>
        </div>
        <div class="mb-6">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Tautan Google Maps (Opsional)</label>
            <input type="url" name="maps_url" placeholder="https://maps.google.com/..." class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border">
            <p class="text-[10px] text-on-surface-variant mt-1">Jika diisi, chatbot otomatis melampirkan tombol penunjuk arah di bawah jawaban.</p>
        </div>
...
<div class="modal" id="editModal">
    <h3 class="font-headline text-2xl font-bold mb-4 text-on-surface">Edit Data</h3>
    <form action="proses_kb.php?action=edit" method="POST">
        <input type="hidden" name="id" id="edit_id">
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Pertanyaan / Kata Kunci</label>
            <textarea name="question" id="edit_question" rows="3" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required></textarea>
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Jawaban Edukatif</label>
            <textarea name="answer" id="edit_answer" rows="5" class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border" required></textarea>
        </div>
        <div class="mb-6">
            <label class="block text-sm font-medium text-on-surface-variant mb-1">Tautan Google Maps (Opsional)</label>
            <input type="url" name="maps_url" id="edit_maps_url" placeholder="https://maps.google.com/..." class="w-full rounded-lg border-outline-variant/50 focus:border-primary focus:ring-primary text-sm p-3 border">
            <p class="text-[10px] text-on-surface-variant mt-1">Kosongkan jika tidak relevan dengan lokasi.</p>
        </div>
        <div class="flex justify-end gap-3">
            <button type="button" onclick="closeModals()" class="px-4 py-2 text-sm font-bold text-on-surface-variant hover:bg-surface-container-high rounded-full">Batal</button>
...
<script>
    ...
    function openEditModal(id, question, answer, maps_url) {
        document.getElementById('edit_id').value = id;
        document.getElementById('edit_question').value = question;
        document.getElementById('edit_answer').value = answer;
        document.getElementById('edit_maps_url').value = maps_url;
        openModal('editModal');
    }
</script>
